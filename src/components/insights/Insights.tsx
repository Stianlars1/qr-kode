...
export const Insights = () => {
    const { genererte, nedlastninger, generertMedBilde, isLoading } = useFetchInsights()
    return (
        <>
            <div className="insights-container">
                ...
                <div className="insights-sliding-text">
                    ...
                    <span>Unike genereringer med bilde: {isLoading ? '⏳' : generertMedBilde}</span>
                </div>
            </div>
        </>
    )
...
const useFetchInsights = (): UseFetchInsights => {
    const query = useQuery<Insights>({ queryKey: ['insights'], queryFn: async () => getInsights() })
    return { genererte: query.data?.generated || 0, nedlastninger: query.data?.downloads || 0, generertMedBilde: query.data?.addedLogo || 0, isLoading: query.isLoading }
}
